...
- [a treasury total is absurd (trillions of USDC); grouping or pricing by token symbol](lessons/token-symbols-are-not-identifiers.md) — 42 mainnet contracts call themselves USDC, decimals and all; symbols are attacker-controlled text, so key on (chain_id, token_address) everywhere.
- [census/backfill decelerates as history grows; ~11s per target with clean batches and fast RPC](lessons/readback-full-scan-on-attempt-id.md) — read-backs filtered on `attempt_id` alone can't use the sort-key prefix and full-scan with FINAL; 8701ms -> 62ms once the prefix is carried.
- [tempted to skip "airdrop-shaped" discovered tokens to cut census cost](lessons/airdrop-shape-is-not-a-spam-filter.md) — measured: that filter drops 3/4 of the tokens holding real value; one-shot means "never moved again", not "worthless".
- ["Error serializing column `observed_sum_raw` into ... UInt256" / OverflowError](lessons/observed-sum-overflows-uint256.md) — the sum of valid uint256 balances is not itself uint256-bounded; discovered spam tokens hit this. Check against UINT256_MAX and block publication with a named reason.
...
- [backfill retry pod publishes ~nothing yet exits 1 on discovery (code 241 on discovery_ranges)](lessons/backfill-retry-burns-backoff-on-discovery.md) — census ran discovery for every full_holders token before the skip gate; now the gate runs first and discovery covers only pending targets.
